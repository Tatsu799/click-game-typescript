...
// 一般的な設計パターンの一つです。
// 大雑把に言うと、モデル(M)、ビュー(V)、コントローラー(C)の3つに担当を分けて実装する設計のパターンの理解です。

// ・それぞれの役割は？
// モデル（Model）
// モデルは、データやロジックを担当するコンポーネント。
...
// また、ビューからの操作やイベントに応じて、適切なアクションを実行し、モデルやビューの状態を更新します。
// コントローラは、モデルとビューの間のインターフェースとして機能し、データのフローとアプリケーションの振る舞いを制御します。
...
//個人的な考え
// MVCモデルについて調べたり、実際にそれに則って実装してみたものの、正直「この実装はmodel？それともController？どっちだ？」
// といったように悩むことが多々あるかと思いました。
// ある程度大まかには、決まりはあるとは思いますが、人によってそれぞれの役割のわけ方など変わる部分もあるのかなと。
//　一人で開発する分には大丈夫かもですが、多人数で開発する場合はお互いの認識を合わせておく必要があるのかなと感じました。
